...
import pytest
import os
# DEMO CODE REMOVED: from unittest.mock import patch, mock_open, MagicMock, call
import tempfile
...
@pytest.fixture
# DEMO CODE REMOVED: def mock_environment(monkeypatch):
...
# DEMO CODE REMOVED: @patch("builtins.open", new_callable=mock_open, read_data="Test prompt question")
# DEMO CODE REMOVED: def test_ceo_and_board_prompt_success(self, mock_file, mock_prompt, mock_prompt_from_file_to_file, mock_environment, tmpdir):
        """Test successful CEO and board prompt execution."""
# DEMO CODE REMOVED: # Set up mocks
# DEMO CODE REMOVED: mock_prompt_from_file_to_file.return_value = [
            str(Path(tmpdir) / "test_a_claude-3.md"),
            str(Path(tmpdir) / "test_o_gpt-4o.md")
        ]
# DEMO CODE REMOVED: mock_prompt.return_value = ["# CEO Decision\n\nThis is the CEO decision content."]
        ...
        # Assertions
# DEMO CODE REMOVED: mock_prompt_from_file_to_file.assert_called_once_with(
            input_file,
            ["a:claude-3", "o:gpt-4o"],
            str(tmpdir)
        )
...
# DEMO CODE REMOVED: mock_prompt.assert_called_once()
# DEMO CODE REMOVED: prompt_arg = mock_prompt.call_args[0][0]
...
# DEMO CODE REMOVED: assert mock_file.call_count >= 2  # Should be called at least twice - once for prompt XML and once for decision
...
# DEMO CODE REMOVED: for call_args in mock_file.call_args_list:
...
# DEMO CODE REMOVED: for call_args in mock_file.call_args_list:
    ...
    @patch("just_prompt.molecules.ceo_and_board_prompt.prompt")
# DEMO CODE REMOVED: @patch("builtins.open", new_callable=mock_open, read_data="Test prompt question")
# DEMO CODE REMOVED: def test_ceo_and_board_prompt_with_defaults(self, mock_file, mock_prompt, mock_prompt_from_file_to_file, mock_environment, tmpdir):
...
# DEMO CODE REMOVED: mock_prompt.assert_called_once()
# DEMO CODE REMOVED: assert mock_prompt.call_args[0][1] == [DEFAULT_CEO_MODEL]
...
# DEMO CODE REMOVED: assert mock_file.call_count >= 2  # Once for prompt XML and once for decision

    @patch("just_prompt.molecules.ceo_and_board_prompt.prompt_from_file_to_file")
    @patch("just_prompt.molecules.ceo_and_board_prompt.prompt")
# DEMO CODE REMOVED: def test_ceo_and_board_prompt_file_not_found(self, mock_prompt, mock_prompt_from_file_to_file, mock_environment):
        """Test error handling when input file is not found."""
        non_existent_file = "non_existent_file.txt"
        
# DEMO CODE REMOVED: # Mock file not found error
# DEMO CODE REMOVED: mock_open_instance = mock_open()
# DEMO CODE REMOVED: mock_open_instance.side_effect = FileNotFoundError(f"File not found: {non_existent_file}")
        
# DEMO CODE REMOVED: with patch("builtins.open", mock_open_instance):

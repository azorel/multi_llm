...
import pytest
import os
# DEMO CODE REMOVED: from unittest.mock import patch
...
# DEMO CODE REMOVED: # Use mocked environment variables with a mix of valid, empty, and missing keys
...
# DEMO CODE REMOVED: # This depends on the actual implementation. Since we're mocking the environment,
...
# DEMO CODE REMOVED: # Use mocked environment variables with no API keys
...
# DEMO CODE REMOVED: # Mock the validate_provider_api_keys function to return a controlled result
# DEMO CODE REMOVED: mock_availability = {
        "openai": True,
        "anthropic": False,
        "gemini": True,
        "groq": False,
...
# DEMO CODE REMOVED: return_value=mock_availability):
        
# DEMO CODE REMOVED: # Mock the logger to verify the log messages
# DEMO CODE REMOVED: with patch('just_prompt.atoms.shared.validator.logger') as mock_logger:
...
# DEMO CODE REMOVED: mock_logger.info.assert_called_once()
# DEMO CODE REMOVED: info_call_args = mock_logger.info.call_args[0][0]
...
# DEMO CODE REMOVED: assert mock_logger.warning.call_count >= 2
            
            # Check that the first warning is about missing API keys
# DEMO CODE REMOVED: warning_calls = [call[0][0] for call in mock_logger.warning.call_args_list]

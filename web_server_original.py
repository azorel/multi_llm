...
@app.route('/')
def index():
    """Home page - Redirect to modern dashboard."""
...
@app.route('/api/recent-repos')
def api_recent_repos():
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM knowledge_hub 
            WHERE category LIKE '%GitHub Repository%'
        ...
        repos = []
        for row in cursor.fetchall():
            title, source, stars, forks, language, owner = row
            repo_name = title.replace(' - GitHub Repository', '')
            repos.append({
                'name': repo_name,
                'stars': f"{stars//1000}K" if stars and stars > 1000 else str(stars or 0),
                'forks': f"{forks//1000}K" if forks and forks > 1000 else str(forks or 0),
        ...
        logger.error(f"Error getting recent repos: {e}")
        return jsonify({
            'success': False,
            'repos': []
        })
...
@app.route('/todays-cc')
def todays_cc():
    """Redirect to modern Today's CC."""
    return redirect(url_for('todays_cc_modern'))

@app.route('/todays-cc-modern')
def todays_cc_modern():
    """Modern Today's Command Center page."""
    tasks = db.get_table_data('todays_cc')
    today = datetime.now().strftime('%Y-%m-%d')
    return render_template('todays_cc_modern.html', tasks=tasks, today=today)

@app.route('/knowledge-hub')
def knowledge_hub():
    """Redirect to modern Knowledge Hub."""
    return redirect(url_for('knowledge_hub_modern'))

@app.route('/knowledge-hub-modern')
def knowledge_hub_modern():
    """Modern Knowledge Hub database page."""
    entries = db.get_table_data('knowledge_hub')
    return render_template('knowledge_hub_modern.html', entries=entries)

@app.route('/agent-command-center')
def agent_command_center():
    """Redirect to modern Agent Command Center."""
    return redirect(url_for('agent_command_center_modern'))

@app.route('/agent-command-center-modern')
...
def prompt_library():
    """Redirect to modern Prompt Library."""
    return redirect(url_for('prompt_library_modern'))

@app.route('/prompt-library-modern')
...
def github_users():
    ...
    return redirect(url_for('github_users_modern'))

@app.route('/github-users-modern')
...
@app.route('/add-github-repo', methods=['POST'])
def add_github_repo():
    """Add a GitHub repository to the knowledge hub."""
    try:
        ...
        # Import and use the GitHub repository processor
        from github_repository_processor import GitHubRepositoryProcessor
        
        processor = GitHubRepositoryProcessor()
        success, message, stars = processor.process_repository(repo_url)
...
def github_repos():
    ...
    return redirect(url_for('github_repos_modern'))

@app.route('/github-repos-modern')
def github_repos_modern():
    """Modern GitHub repositories page."""
    try:
        conn = sqlite3.connect('lifeos_local.db')
        cursor = conn.cursor()
        
        # Get GitHub repositories from knowledge hub
        cursor.execute("""
            SELECT id, title, source, category, tags, created_date, 
                   github_stars, github_forks, github_language, github_owner
            FROM knowledge_hub 
            WHERE category LIKE '%GitHub Repository%'
            ORDER BY github_stars DESC NULLS LAST, created_date DESC
        """)
        ...
        for row in cursor.fetchall():
            repos.append({
                ...
                'tags': row[4],
                'created_date': row[5],
                'github_stars': row[6] or 0,
                'github_forks': row[7] or 0,
                'github_language': row[8] or 'Unknown',
                'github_owner': row[9] or 'Unknown'
            })
        ...
        logger.error(f"Error loading modern GitHub repositories: {e}")
        return f"Error: {e}", 500

@app.route('/process-github-user', methods=['POST'])
def process_github_user():
    """Process a GitHub user's repositories."""
    try:
        data = request.get_json()
        ...
        if response.status_code != 200:
            return jsonify({
                'success': False,
                'message': f'Failed to fetch repositories for {username}'
            })
        
        repos = response.json()
        
        # Process top repositories (by stars)
        ...
        return jsonify({
            'success': True,
            'message': f'Processed {processed_count} repositories for {username}',
...
def youtube_channels():
    ...
    return redirect(url_for('youtube_channels_modern'))

@app.route('/youtube-channels-modern')
...
def shopping_list():
    """Redirect to modern version."""
    return redirect(url_for('shopping_list_modern'))

@app.route('/shopping-list-modern')
...
def tasks():
    """Redirect to modern version."""
    return redirect(url_for('tasks_modern'))

@app.route('/tasks-modern')
def tasks_modern():
    """Modern Tasks page."""
...
def habits():
    """Redirect to modern version."""
    return redirect(url_for('habits_modern'))

@app.route('/habits-modern')
def habits_modern():
    """Modern Habits page."""
...
def books():
    """Redirect to modern version."""
    return redirect(url_for('books_modern'))

@app.route('/books-modern')
def books_modern():
    """Modern Books database page."""
...
def journals():
    """Redirect to modern version."""
    return redirect(url_for('journals_modern'))

@app.route('/journals-modern')
def journals_modern():
...
def notes():
    """Redirect to modern version."""
    return redirect(url_for('notes_modern'))

@app.route('/notes-modern')
def notes_modern():
    """Modern Notes page."""
...
def maintenance_schedule():
    """Redirect to modern version."""
    return redirect(url_for('maintenance_schedule_modern'))

@app.route('/maintenance-schedule-modern')
...
def model_testing():
    """Redirect to modern version."""
    return redirect(url_for('model_testing_modern'))

@app.route('/model-testing-modern')
def model_testing_modern():
    """Modern Model Testing Dashboard."""
...
def voice_commands():
    """Redirect to modern version."""
    return redirect(url_for('voice_commands_modern'))

@app.route('/voice-commands-modern')
...
def workflow_templates():
    """Redirect to modern version."""
    return redirect(url_for('workflow_templates_modern'))

@app.route('/workflow-templates-modern')
...
def provider_status():
    """Redirect to modern version."""
    return redirect(url_for('provider_status_modern'))

@app.route('/provider-status-modern')
def provider_status_modern():
    """Modern LLM Provider Status Dashboard."""
    try:
...
def agent_results():
    """Redirect to modern version."""
    return redirect(url_for('agent_results_modern'))

@app.route('/agent-results-modern')
...
def cost_tracking():
    """Redirect to modern version."""
    return redirect(url_for('cost_tracking_modern'))

@app.route('/cost-tracking-modern')
...
def server_status():
    """Redirect to modern version."""
    return redirect(url_for('server_status_modern'))

@app.route('/server-status-modern')
def server_status_modern():
    """Modern Server Status Dashboard page."""
    import subprocess
    import psutil
    import os
    import time
...
def active_agents():
    """Redirect to modern version."""
    return redirect(url_for('active_agents_modern'))
...
def autonomous_monitor():
    """Redirect to modern version."""
    return redirect(url_for('autonomous_monitor_modern'))
...
def edit_item():
    """Edit existing item in any table."""
    data = request.json
    table = data.get('table')
    record_id = data.get('id')
    ...
    success = db.update_record(table, record_id, updates)
    
    return jsonify({'success': success})

@app.route('/delete-item', methods=['POST'])
def delete_item():
    """Delete item from any table."""
    data = request.json
    table = data.get('table')
    record_id = data.get('id')
...
def get_item():
    """Get single item from any table."""
    ...
    item = db.get_record(table, int(record_id))
    
    return jsonify({'success': bool(item), 'item': item})
...
def business_empire():
    """Redirect to modern version."""
    return redirect(url_for('business_empire_modern'))

@app.route('/business-opportunities')
def business_opportunities():
    """Redirect to modern version."""
    return redirect(url_for('business_opportunities_modern'))

@app.route('/business-projects')
def business_projects():
    """Redirect to modern version."""
    return redirect(url_for('business_projects_modern'))
...
def run_server():
    """Run the web server."""
    print("🚀 Starting LifeOS Modern Web Interface")
    print("🎨 Modern design system with clean interface available at:")
    print("   http://localhost:5000")
    print()
    print("=== ✨ MODERN INTERFACE ===")
    print("🎯 Modern Dashboard: http://localhost:5000/modern-dashboard")
    print("📦 GitHub Repositories: http://localhost:5000/github-repos-modern")
    ...
    print("📅 Today's Tasks: http://localhost:5000/todays-cc")
    ...
    print()
    print("=== 🏢 BUSINESS EMPIRE ===")
    print("👑 Business Empire: http://localhost:5000/business-empire")
    print("💡 Opportunities: http://localhost:5000/business-opportunities")
    print("🚀 Projects: http://localhost:5000/business-projects")
    print("💰 Revenue: http://localhost:5000/business-revenue")
    print("🤖 Business Agents: http://localhost:5000/business-agents")
    print()
    print("=== 🔥 NEW FEATURES ===")
    print("📦 GitHub Repository Import with Bulk Processing")
    print("🎨 Modern UI with Inter Font & Clean Design")
    print("📊 Real-time Stats & Activity Feeds")
    print("⚡ Fast Repository Processing & Analysis")
...
# MODERN ROUTE VERSIONS

@app.route('/active-agents-modern')
def active_agents_modern():
    """Modern active agents dashboard with real-time monitoring"""
    try:
...
@app.route('/autonomous-monitor-modern')
def autonomous_monitor_modern():
    """Modern real-time monitoring of autonomous system activity."""
    import sqlite3
    import json
    from datetime import datetime, timedelta
...
@app.route('/business-empire-modern')
...
@app.route('/business-opportunities-modern')
...
@app.route('/business-projects-modern')
